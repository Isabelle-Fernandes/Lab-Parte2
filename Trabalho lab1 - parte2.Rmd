...
Esse teste é utilizado para verificar se existe associação entre as variáveis grupo com cada uma das outras variáveis qualitativas.

Hipóteses:

$H_0$: As variáveis **grupo** com cada uma das outras variáveis são independentes

$H_1$: As variáveis **grupo** com cada uma das outras variáveis são independentes estão associadas
...
# 4 - Modelo de regressão logistica

<div style="text-align: justify">

A análise de regressão é uma ferramenta estatística que nos permite explorar as relações entre variáveis, identificar fatores significativos e quantificar o impacto desses fatores em uma variável de interesse. Como temos uma variável de interesse com duas categorias, vamos utilizar uma regressão logistica.
...
M1 <- glm(Grupo ~ Filmes + `Visita a parques, praças e espaços na natureza exibidos na tela` + Satisfação + 
                `Visita a patrimônios culturais exibidos na tela` + 
        `Desfrute da gastronomia retratada na tela, incluindo alimentos e bebidas`, data = variaveis,
               family = binomial(link = "logit"))

#Modelo retirando variável não significativa

M2 <- glm(Grupo ~ Filmes + `Visita a parques, praças e espaços na natureza exibidos na tela` + Satisfação + 
        `Desfrute da gastronomia retratada na tela, incluindo alimentos e bebidas`, data = variaveis,
               family = binomial(link = "logit"))

#Modelo utilizando as variaveis com p-valor < 0.05 no teste qui-quadrado

M3 <- glm(Grupo ~ Filmes + `Visita a parques, praças e espaços na natureza exibidos na tela` +  
        `Desfrute da gastronomia retratada na tela, incluindo alimentos e bebidas`, data = variaveis,
               family = binomial(link = "logit"))
```
...
deltaDevi <- M3$deviance - M2$deviance # segue distribuição qui-quadrado com (M3$df.residual - M$df.residual) de liberdade

valorp <- 1-pchisq(deltaDevi, 1) # probabilidade de ser maior que a deltaDevi
```


A estatística $D_q$ (deviance do modelo com menos parâmetros) $D_p$ (deviance do modelo com mais parâmetros), onde q e p é número de parâmetros dos modelos, respectivamente, é útil como medida de variação dos dados explicada pelos termos que estão no modelo maior e não estão no modelo menor.
...
Como o valor-p deu 0.014, rejeitamos a hipótese nula e ficamos com o modelo com mais parâmetros.

Vamos conferir se a deviance é moderada:
...
O modelo com menor AIC foi o modelo2
...
Como desejado, todas as variáveis do modelo tem uma significância estatística e são relevantes para a interpretação do mesmo.

```{r echo=FALSE}
coef_modelo2 <- exp(coef(M2)[2:5])

names(coef_modelo2) <- c("Filmes", "Visita a parques, praças e espaços na natureza exibidos na tela",
                         "Satisfeito/ Indiferente/ Muito insatisfeito", "Desfrute da gastronomia retratada na tela")

kable(coef_modelo2, caption = "Tabela dos coeficientes para interpretacao")


```

```{r}
eta = predict(M2) # preditor linear
odds = exp(eta) # razão de chance
ychapeu <- odds/(1+odds)

Turista <- c(1:101,103:117) # o individuo 102 foi omitido no ajuste do modelo pois estava com NA, 

posicao <- NULL #vetor posição dos individuos que tem o valor real "Específica" 
...
posicao2 <- NULL #vetor posição dos individuos que o modelo previu como "Específica"

for (i in 1:116) {
  if (ychapeu[i]>0.5) {
    posicao2 <- append(posicao2, i)
  }
}
```

Turistas que tem o valor real Específica
...
Turistas que o modelo classificou como Específica
...
data.frame(Turista, ychapeu) %>% 
  ggplot(aes(x=Turista, y=ychapeu)) +
  geom_point() +
...
variaveis_sem_pontos_influentes <- variaveis[-106,] #retirando o ponto 106

M4 <- glm(Grupo ~ Filmes + `Visita a parques, praças e espaços na natureza exibidos na tela` + Satisfação + 
        `Desfrute da gastronomia retratada na tela, incluindo alimentos e bebidas`, data = variaveis_sem_pontos_influentes,
        family = binomial(link = "logit"))

summary(M4)

eta = predict(M4) # preditor linear
odds = exp(eta) # razão de chance
ychapeu <- odds/(1+odds)

Turista <- c(1:101,103:105,107:117) # o individuo 102 e 106 foi omitido no ajuste do modelo.

posicao <- NULL #vetor posição dos individuos que tem o valor real "Específica" 

for (i in 1:115) {
  if (variaveis_sem_pontos_influentes[i,1]=="1") {
    posicao <- append(posicao, i)
