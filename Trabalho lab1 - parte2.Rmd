...
banco[] <- lapply(banco, factor)
str(banco)
<<<<<<< HEAD


modelo1 <- glm(banco$Grupo ~ banco$Filmes 
               + banco$`Visita a parques, praças e espaços na natureza exibidos na tela` + banco$Satisfação + 
...
modelo2 <- glm(banco$Grupo ~ banco$Filmes 
               + banco$`Visita a parques, praças e espaços na natureza exibidos na tela` + banco$Satisfação + 
                 banco$`Desfrute da gastronomia retratada na tela, incluindo alimentos e bebidas`,
               family = binomial(link = "logit"))
...
modelo3 <- glm(banco$Grupo ~ banco$Filmes 
               + banco$`Visita a parques, praças e espaços na natureza exibidos na tela` + banco$Satisfação,
               family = binomial(link = "logit"))
...
summary(modelo2)$coefficients[,c(1,4)]
...
```{r echo=FALSE}
coef_modelo2 <- exp(coef(modelo2)[2:5])
...
O modelo que se mostrou mais parcimonioso foi justamente aquele que utilizava as variáveis mais significativas no teste qui-quadrado para explicar o grupo dos individuos. Dessa forma o modelo final ficou como:
...
```{r echo=FALSE}
coef_modelo2 <- exp(coef(modelo2)[2:3])
names(coef_modelo2) <- c("Filmes", "Visita")
kable(coef_modelo2, caption = "Tabela dos coeficientes para interpretacao")
```

A interpretação dos coeficientes do modelo se da por:

- Como exp(coeficiente) da variável "Filmes" é 7.94, isso implica que um 'Sim' em "Filmes" aumenta as chances do "Grupo" ser 'Especifica' em aproximandamente 8 vezes.
- Como exp(coeficiente) da variável "Visita a parques..." é 4.35, isso implica que um 'Sim' aumenta as chances do "Grupo" ser 'Especifica' em aproximadamente 4 vezes.

>>>>>>> 98746bb42e360f876dac2eca1cc18c63a6af8524
